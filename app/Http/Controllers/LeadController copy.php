...
class LeadController extends Controller
{
    ...
    {
        $known = [
            'ProspectID','ProspectAutoId','StatusCode','StatusReason','IsLead','NotableEvent','NotableEventdate','LastVisitDate',
            'FirstName','LastName','EmailAddress','Phone','Company','Source','SourceCampaign','JobTitle','Score','EngagementScore',
            'ProspectStage','OwnerId','OwnerIdName','OwnerIdEmailAddress','CreatedBy','CreatedByName','CreatedOn','LeadConversionDate',
            'ModifiedBy','ModifiedByName','ModifiedOn','mx_City','mx_Course_Interested','mx_Education_Completed','mx_Ad_Name',
            'mx_campaign_Id','mx_Adset_Name','mx_Facebook_Form','mx_Facebook_Page','mx_Status','mx_Outcome','mx_Lead_Course',
            'mx_AD_Network','mx_utm_creative_id','mx_FB_LeadGen_ID','mx_Program_Type','mx_Source_Category','mx_Courses_Category',
            'mx_Total_Calls_in_Lead','mx_Level_of_interest','mx_Primary_reason_for_course'
        ];
        $toInsert = [];
        foreach ($known as $col) {
            $toInsert[$col] = $leadData[$col] ?? null;
    ...
    public function hydrateLeadsFromSearch(Request $request)
    {
        ...
        // Larger chunk and pool sizes dramatically improve throughput; can be tuned per server limits
        $dbChunk = max(200, (int) $request->query('chunkSize', 1000));
        $poolSize = max(10, (int) $request->query('poolSize', 30));

        $total = \App\Models\TeamConfig::whereNotNull('ProspectID')->count();
        // dd($total);
        $processed = 0;
        $stored = 0;
        $errors = 0;
        ...
        \App\Models\TeamConfig::whereNotNull('ProspectID')
            ...
            ->orderBy('id')
            ->chunkById($dbChunk, function ($rows) use ($concurrent, $poolSize, &$processed, &$stored, &$errors) {
                // Deduplicate IDs within the chunk
                $ids = $rows->pluck('ProspectID')->filter()->unique()->values()->all();
                $processed += count($ids);

                Log::info('[HydrateLeads] Processing chunk', [
                    'count' => count($ids),
                    'processed' => $processed,
                ]);
                ...
                // Skip IDs already present in leads to reduce API calls
                ...
                if ($concurrent) {
                    // Concurrent mode: use Http::pool for speed
                    $accessKey = env('LEADSQUARED_ACCESS_KEY');
                    $secretKey = env('LEADSQUARED_SECRET_KEY');
                    foreach (array_chunk($ids, $poolSize) as $poolIds) {
                        $responses = Http::pool(function ($pool) use ($poolIds, $accessKey, $secretKey) {
                            foreach ($poolIds as $id) {
                                $url = "https://api-in21.leadsquared.com/v2/LeadManagement.svc/Leads.GetById?accessKey={$accessKey}&secretKey={$secretKey}&id={$id}";
                                // Tighten timeouts and retries for throughput; adjust if API is unstable
                                $pool->as($id)->timeout(10)->retry(1, 150)->get($url);
                            }
                        });
                        // Collect rows for batch upsert to reduce DB roundtrips
                        $toUpsert = [];
                        foreach ($responses as $id => $response) {
                            try {
                                if ($response && method_exists($response, 'ok') && $response->ok()) {
                        ...
                        if (!empty($toUpsert)) {
                            // Determine update columns (exclude unique key)
                        ...
                        // Gentle throttle between pools to avoid provider throttling
                        usleep(100000);
                    }
                } else {
                    // Serial mode calling existing method as requested
                    foreach ($ids as $id) {
                        try {
                            $toInsert = $this->testDetailsByGetId($id);
                            if (is_array($toInsert) && !empty($toInsert['ProspectID'])) {
                                // Small buffer then upsert singly (serial path only used when explicitly requested)
